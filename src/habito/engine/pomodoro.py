...
class PomodoroEngine:
    ...
    def _session(self) -> UUID:
        """The running session's id, which every event of that session carries.

        Reading it outside a session is a bug rather than a state to handle: nothing emits
        before :meth:`start`, so a ``None`` here would mean a transition fired from idle.
        """
    ...
    def _end_session(self) -> None:
        self._sink(
            SessionEnded(
                ...
                habit=self._habit,
                session_id=self._session,
                total_work_seconds=self._accumulated_work,
            )
        )
